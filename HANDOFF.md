...
- **APK distribution (2026-09-24):** the main repo **`gitJMDR/einp-airsurvey` is PUBLIC** (history rewritten clean — versions/ and the morning report purged from all commits; reference material/ and APKs were never committed). Releases carry the APKs (GitHub Releases allow 2 GB files — the 100 MB limit is only for repo contents): `gh release create vX.Y.Z --title vX.Y.Z --notes "..." <apk path>` per build. Crew link: `github.com/gitJMDR/einp-airsurvey/releases` (also in USER-GUIDE's Setup section). EAS artifact links expire (~30 days free tier) — release attachments are the durable public link. The interim `airsurvey-releases` repo is retired/deleted
...
6. ~~MapLibre camera controls~~ **Reworked 2026-09-25 for v0.2.0** (field feedback): EVERY N↑/H↑ tap sets mode + re-centres + re-engages follow (Camera `trackUserLocation`: "course" for H↑, "default" for N↑ — the follow prop must be dropped-then-reapplied to re-engage, same-value prop changes are no-ops); manual pan (onRegionDidChange with userInteraction + centre moved) ends follow until the next tap; double-tap adds zoomTo(default five-line zoom); snap-backs ≈150 ms ("smooth reads as lag in the cabin"). N↑ additionally easeTo's bearing 0 — "default" follow doesn't level the bearing itself. Earlier state (2026-09-21–24): shared `MapControls.tsx` 2×3 grid (SAT N↑ H↑ | 🏭 + −), waypoint labels on the embedded "Open Sans Semibold" font, 9001+ pins grey, tap-a-pin edits; H↑ verified rotating in a moving vehicle. v0.2.0 also adds: HUD ↑/↓ correction arrows outside tolerance (App.tsx tolArrow, coloured like the value); EXPORT · SAVE TO DEVICE (SAF `Directory.pickDirectoryAsync` — offline file copy; export UI now only on DATA screen); satellite pack maxZoom 17 + 5 km margin (~1 GB — **users must Remove + re-Download packs after upgrading**; the "soft corridor imagery" was z16's ceiling, not missing tiles)
