...
- **APK distribution (2026-09-24):** public repo **`gitJMDR/airsurvey-releases`** holds versioned releases with the APKs attached (GitHub Releases allow 2 GB files — the 100 MB limit is only for repo contents). `gh release create vX.Y.Z --repo gitJMDR/airsurvey-releases --title vX.Y.Z --notes "..." <apk path>` per build; README there carries crew install instructions. EAS artifact links expire (~30 days free tier) — release attachments are the durable public link (linked from USER-GUIDE's Setup section)
